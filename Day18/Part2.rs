...
fn solve(reader: &mut Reader, writer: &mut BufWriter<Stdout>) {
    ...
    let mut extra_byte_cnt: usize = 0;
    let mut extra_bytes: Vec<(usize, usize)> = Vec::new();

    for line in input.lines() {
        let position: Vec<usize> = line.split(",").map(|p| p.parse().unwrap()).collect();
        let (x, y) = (position[1], position[0]);
        if extra_byte_cnt < MAX_BYTES {
            grid[x][y] = '#';
            extra_byte_cnt += 1;
        } else {
            extra_bytes.push((position[1], position[0]));
        }
    }

    for &extra_byte in extra_bytes.iter() {
        grid[extra_byte.0][extra_byte.1] = '#';
        ...
        let mut did_find_path: bool = false;
        ...
        if !did_find_path {
            writeln!(writer, "{},{}", extra_byte.1, extra_byte.0).ok();
            break;
        }
    }
}
