...
fn solve(reader: &mut Reader, writer: &mut BufWriter<Stdout>) {
    ...
    for desired_design in desired_designs.iter() {
        let mut exists_pattern: Vec<Vec<i64>> =
            vec![vec![0; desired_design.len()]; desired_design.len()];
        ...
        for i in 0..desired_design.len() {
            ...
            for j in i..desired_design.len() {
                ...
                if has_pattern.contains(&sub_str.as_str()) {
                    exists_pattern[i][j] += 1;
                }
                for k in i..j {
                    if exists_pattern[i][k] > 0 && has_pattern.contains(&sub_str[k - i + 1..]) {
                        exists_pattern[i][j] += exists_pattern[i][k];
                    }
                }
            }
        }
        total += exists_pattern[0][desired_design.len() - 1];
    }
