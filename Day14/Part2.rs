...
fn solve(reader: &mut Reader, writer: &mut BufWriter<Stdout>) {
    ...
    let mut bathroom: Vec<Vec<i64>> = vec![vec![0; BATHROOM_WIDTH]; BATHROOM_HEIGHT];
    ...
    for (&robot_position, &robot_velocity) in robot_positions.iter().zip(robot_velocities.iter()) {
        ...
        bathroom[robot_position.0 as usize][robot_position.1 as usize] += 1;
    }

    let mut needed_iterations: i64 = 0;
    while let Some((robot_position, robot_velocity, iterations)) = simulation_queue.pop_front() {
        let mut max_consecutive: i64 = 0;
        let mut consecutive: i64 = 0;
        for j in 0..BATHROOM_WIDTH {
            if bathroom[51][j] > 0 {
                if consecutive == 0 {
                    consecutive = 1;
                } else {
        ...
        if max_consecutive > 7 {
            needed_iterations = iterations + 1;
            break;
        }
        ...
        bathroom[robot_position.0 as usize][robot_position.1 as usize] -= 1;
        bathroom[next_robot_position.0 as usize][next_robot_position.1 as usize] += 1;
    }

    writeln!(writer, "{}", needed_iterations).ok();
}
