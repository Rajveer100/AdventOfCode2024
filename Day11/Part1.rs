    ...
    for _ in 0..25 {
        ...
        while i < stones.len() {
            if stones[i] == "0" {
                ...
                let stone_to_split = stones.remove(i);

                let l_stone = stone_to_split[stone_to_split.len() / 2..]
                    .chars()
                    .skip_while(|&ch| ch == '0')
                    .collect::<String>();
                if l_stone.is_empty() {
                    stones.insert(i, "0".to_owned());
                } else {
                    stones.insert(i, l_stone);
                }

                let r_stone = stone_to_split[..stone_to_split.len() / 2]
                    .chars()
                    .skip_while(|&ch| ch == '0')
                    .collect::<String>();
                if r_stone.is_empty() {
                    ...
                    stones.insert(i, r_stone);
                }

                i += 2;
            } else {
                stones[i] = (stones[i].parse::<i64>().unwrap() * 2024).to_string();
                i += 1;
            }
        }
    }

    writeln!(writer, "{}", stones.len()).ok();
}
