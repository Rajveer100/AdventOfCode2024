...
fn solve(reader: &mut Reader, writer: &mut BufWriter<Stdout>) {
    ...
    let mut dist: Vec<Vec<i64>> = vec![vec![0; race_track[0].len()]; race_track.len()];
    ...
    while let Some((pos, cheats, time)) = race_q.pop_front() {
        ...
        for &(dx, dy) in DIR4.iter() {
                ...
                && race_track[nx as usize][ny as usize] != '#'
            {
                ...
                dist[nx as usize][ny as usize] = time + 1;
                race_q.push_back(((nx, ny), 0, time + 1));
            }
        }
    }

    visited = vec![vec![false; race_track[0].len()]; race_track.len()];

    race_q.push_back((src, 0, 0));
    visited[src.0 as usize][src.1 as usize] = true;

    let mut satisfied_cheat_time_cnt: i64 = 0;
    while let Some((pos, cheats, time)) = race_q.pop_front() {
        if pos == dest {
            continue;
        }
        ...
        for &(dx, dy) in DIR4.iter() {
            ...
            if nx >= 0
                ...
                && !visited[nx as usize][ny as usize]
            {
                if race_track[x as usize][y as usize] == '#'
                    && race_track[nx as usize][ny as usize] != '#'
                {
                    let cheat_time = time + original_time - dist[nx as usize][ny as usize];
                    if original_time - cheat_time >= 100 {
                        satisfied_cheat_time_cnt += 1;
                    }
                    continue;
                }
                if race_track[nx as usize][ny as usize] != '#' {
                    visited[nx as usize][ny as usize] = true;
                    race_q.push_back(((nx, ny), 0, time + 1));
                } else if cheats == 0 {
                    visited[nx as usize][ny as usize] = true;
                    race_q.push_back(((nx, ny), cheats + 1, time + 1));
                }
            }
        }
    }

    writeln!(writer, "{}", satisfied_cheat_time_cnt).ok();
}
