...
fn solve(reader: &mut Reader, writer: &mut BufWriter<Stdout>) {
    let path = Path::new("day23_input.txt");
    ...
    for line in input.lines() {
        ...
        connections.entry(nodes[0]).or_default().push(nodes[1]);
        connections.entry(nodes[1]).or_default().push(nodes[0]);
    }

    let mut triples: Vec<Vec<&str>> = Vec::new();
    for (&from, to) in connections.iter() {
        for &each_to1 in to.iter() {
            for &each_to2 in connections[&each_to1].iter() {
                if connections[&each_to2].contains(&from) {
                    let mut triple = vec![from, each_to1, each_to2];
                    triple.sort();
                    triples.push(triple);
                }
            }
