...
fn equation_is_valid(equation: &Vec<i64>) -> bool {
    return check_operators(1, equation, equation[1]);
}

fn check_operators(index: usize, equation: &Vec<i64>, result: i64) -> bool {
    if index == equation.len() - 1 {
        return if result == equation[0] { true } else { false };
    }

    return check_operators(index + 1, equation, result + equation[index + 1])
        | check_operators(index + 1, equation, result * equation[index + 1]);
}
