...
fn solve(reader: &mut Reader, writer: &mut BufWriter<Stdout>) {
    let path = Path::new("day22_input.txt");
    ...
    let mut secret_numbers: Vec<i64> = input.lines().map(|line| line.parse().unwrap()).collect();

    let mut result: i64 = 0;
    for &secret_number in secret_numbers.iter() {
        let mut n = secret_number;
        for _ in 0..2000 {
            n = ((n * 64) ^ n) % 16777216;
            n = ((n / 32) ^ n) % 16777216;
            n = ((n * 2048) ^ n) % 16777216;
